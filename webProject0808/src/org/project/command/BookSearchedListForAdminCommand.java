...
public class BookSearchedListForAdminCommand implements ExecuteCommand {
	@Override
	public void executeQuery(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		System.out.println("책 검색(페이징)");
		...
		ArrayList<Book_p01> books = dao.selectPageBooksForSearchDo(category, searching, page);
				
		String url = "";
		
		if(books != null) {
			System.out.println("책 검색 성공(페이징)");
			request.setAttribute("books", books);
			request.setAttribute("totalNum", totalNum);
			request.setAttribute("lastPage", lastPage);
			request.setAttribute("lastSection", lastSection);
			url = "bookListForAdmin.jsp";				

		} else {
			System.out.println("책 검색 실패(페이징)");
			url = "index.mo";
		}
