...
public class BookModifyCommand implements ExecuteCommand {
	@Override
	public void executeQuery(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String org_cover = bookMap.get("org_cover");
			...
			if(cover != null && cover.length() != 0) {
				...
				File oldFile = new File(BOOK_COVER_REPO + "\\" + bookNo + "\\" + org_cover);
				oldFile.delete();
			}
		...
		int rs = dao.modifyBookDo(bookMap);
		
		if(rs == 1) {
			System.out.println("성공");
			url = "bookView.do?bookNo="+bookMap.get("bookNo");
		} else {
